...
- (void)viewDidLoad {
    ...
    [participante1 setPais:@"Guatemala"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PaisCellView * cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    
    
    if(!cell) {
        
        
        NSArray * nibObjects = [[NSBundle mainBundle] loadNibNamed:@"PaisCell" owner:self options:nil];
        
        for(id objetoActual in nibObjects) {
            
            if([objetoActual isKindOfClass:[UITableViewCell class]] && [[objetoActual reuseIdentifier] isEqualToString:cellIdentifier]) {
                
                cell = (PaisCellView *)objetoActual;
    ...
    cell.label.text = [[participantes objectAtIndex:indexPath.row] nombre];
    
    cell.imageView.image = [[participantes objectAtIndex:indexPath.row] imagen];
